...
final class TextClip extends Clip {
  ...
  /// Clip-local word clocks. Non-empty turns the caption into karaoke: the
  /// painter highlights each word as its time arrives, and the exporter
  /// rasterises the clip per frame like any animated title.
  final List<WordTiming> wordTimings;

  bool get isKaraoke => wordTimings.isNotEmpty;
  ...
  factory TextClip.fromJson(Map<String, dynamic> json) => TextClip(
    ...
    wordTimings: [
      for (final w in (json['words'] as List?) ?? const [])
        WordTiming.fromJson((w as Map).cast<String, dynamic>()),
    ],
    label: json['label'] as String?,
    locked: json['locked'] as bool? ?? false,
    enabled: !(json['off'] as bool? ?? false),
